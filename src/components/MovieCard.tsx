  ...
  return (
    <div className="rounded-lg bg-white/5 ring-1 ring-white/10 p-3">
      <div className="flex items-center justify-between mb-2">
        {cinemaNameNode}
        <span
          className={`text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full ring-1 ${
      ...
      <div className="flex flex-wrap gap-1.5">
        {cinema.showings.map((showing) => (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="group/tag flex flex-col items-start gap-0.5 px-2.5 py-1 rounded-md bg-zinc-900/60 hover:bg-zinc-800 ring-1 ring-white/5 hover:ring-white/20 transition"
...
export function MovieCard({ movie, englishTitle }: Props) {
  ...
  return (
    <article className="grid grid-cols-[110px,1fr] md:grid-cols-[160px,1fr] gap-4 p-4 rounded-2xl bg-zinc-900/60 ring-1 ring-white/10 hover:ring-white/20 transition-shadow">
      <a
        href={movie.filmLink}
        target="_blank"
        rel="noopener noreferrer"
        className="block aspect-[2/3] overflow-hidden rounded-lg bg-zinc-800"
      >
        {movie.posterUrl ? (
          <img
            src={movie.posterUrl}
